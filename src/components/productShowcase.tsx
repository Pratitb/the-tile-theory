...
const ProductShowcase = ({ head, whatPills, whatActivePill, whatActiveBtn, getProducts }: ProductShowcase) => {
    return (
        <div className="flex flex-col gap-4 pb-6">
            ...
            <div className="flex md:flex-wrap overflow-x-auto lg:overflow-x-hidden gap-4 lg:max-h-[500px]">{getProducts?.map((product: TileProductKeys) => <ProductCard key={product?.name} imageUrl={product?.imageUrl} name={product?.name} type={product?.type} quality={product?.quality} sizes={product?.sizes} pdf={product?.pdfUrl} />)}</div>
        </div>
    )
}
